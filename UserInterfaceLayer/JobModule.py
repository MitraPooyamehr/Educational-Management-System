...
class JobForm:
    def __init__(self, user_param: User_Model_class, master: tk.Misc = None):
        ...
        self.bll = Job_CRUD_BLL_Class()

        # vars
        self.search_var = None
        self.job_id_var = None
        self.job_title_var = None
    ...
    def job_form_load(self):
        try:
            ...
            self.win.title("Job Management")
            self.win.configure(bg="#111111")

            self.win.update_idletasks()
            sw, sh = self.win.winfo_screenwidth(), self.win.winfo_screenheight()
            W = max(1050, int(sw * 0.75))
            H = max(720, int(sh * 0.75))
            x = sw // 2 - W // 2
            y = sh // 2 - H // 2
            ...
            self.job_id_var = tk.StringVar(master=self.win, value="")
            self.job_title_var = tk.StringVar(master=self.win, value="")
            ...
            ttk.Label(header, text="Job Management", style="Title.TLabel").pack(side="left")
            ...
            ttk.Label(toolbar, textvariable=self.job_id_var, style="Value.TLabel").grid(row=0, column=1, padx=(8, 18),
                                                                                        sticky="w")
            ...
            fields.pack(fill="both", expand=True, padx=14, pady=(8, 14))
            fields.columnconfigure(0, weight=1)

            cell = ttk.Frame(fields, style="Panel.TFrame")
            cell.grid(row=0, column=0, sticky="ew", padx=10, pady=4)

            ttk.Label(cell, text="Job Title", style="Form.TLabel").grid(row=0, column=0, sticky="w")
            ent = ttk.Entry(cell, textvariable=self.job_title_var, style="Form.TEntry", width=40)
            ent.grid(row=1, column=0, sticky="w", pady=(3, 0))
            ...
            ttk.Label(search, text="Search Job:", style="Form.TLabel").grid(row=0, column=0, sticky="w")
            ...
            self.tree = ttk.Treeview(
                ...
                displaycolumns=("Row", "JobTitle")  # مخفی کردن ستون ID در ظاهر
            )
            ...
            self.tree.heading("JobTitle", text="Job Title")

            self.tree.column("Row", width=70, anchor="center")
            self.tree.column("ID", width=0, stretch=False)  # مخفی
            self.tree.column("JobTitle", width=450, anchor="w")

            self.tree.bind("<<TreeviewSelect>>", self._on_tree_select)

            # Context Menu
            self.context_menu = tk.Menu(self.win, tearoff=0, bg="#1a1a1a", fg="white", font=font_sm)
            self.context_menu.add_command(label="Update Job", command=self._update)
            self.context_menu.add_command(label="Delete Job", command=self._delete)
            self.tree.bind("<Button-3>", self._show_context_menu)

            self._refresh_tree()
            self.win.after(60, lambda: paned.sashpos(0, int(H * 0.30)))

            if self._created_root:
                self.win.mainloop()

        except Exception as e:
            messagebox.showerror("Job Form Error", str(e), parent=self._msg_parent())

    # ===================== CRUD =====================
    def _validate(self) -> bool:
        title = (self.job_title_var.get() or "").strip()
        if not title:
            messagebox.showerror("Validation", "Job Title is required.", parent=self._msg_parent())
            return False
        if len(title) > 50:
            messagebox.showerror("Validation", "Job Title must be <= 50 chars.", parent=self._msg_parent())
            return False
        return True
    ...
    def _save(self):
        ...
        try:
            title = (self.job_title_var.get() or "").strip()
            new_id = self.bll.register_job(Job_Model_Class(title))
            if new_id is None:
                messagebox.showerror("Save", "Insert failed.", parent=self._msg_parent())
                return
            self.job_id_var.set(str(new_id))
            messagebox.showinfo("Saved", f"Job saved successfully. ID={new_id}", parent=self._msg_parent())
            self._clear_form()
            self._refresh_tree()
        except Exception as e:
    ...
    def _update(self):
        jid = (self.job_id_var.get() or "").strip()
        if not jid:
        ...
        try:
            title = (self.job_title_var.get() or "").strip()
            ok = self.bll.update_job(Job_Model_Class(title, job_id=int(jid)))
            if not ok:
                messagebox.showerror("Update", "Update failed.", parent=self._msg_parent())
                return
            messagebox.showinfo("Updated", "Job updated successfully.", parent=self._msg_parent())
            self._refresh_tree()
        except Exception as e:
            messagebox.showerror("DB Error", str(e), parent=self._msg_parent())
    ...
    def _delete(self):
        ...
        if not messagebox.askyesno("Confirm", f"Are you sure you want to delete Job ID={jid}?",
                                   parent=self._msg_parent()):
            return

        try:
            ok = self.bll.delete_job(int(jid))
            if not ok:
                messagebox.showerror("Delete", "Delete failed.", parent=self._msg_parent())
                return
            messagebox.showinfo("Deleted", "Job deleted successfully.", parent=self._msg_parent())
            self._clear_form()
            self._refresh_tree()
        except Exception as e:
            if "REFERENCE constraint" in str(e):
                msg = "Cannot delete this job because it is assigned to one or more employees."
    ...
    def _refresh_tree(self):
        ...
        for idx, job in enumerate(data, start=1):
            # استفاده از job.ID به عنوان iid
            self.tree.insert("", "end", iid=str(job.ID), values=[idx, job.ID, job.JobTitle])

        if not (self.job_id_var.get() or "").strip():
            self._set_selection_state(False)
    ...
    def _on_tree_select(self, _event):
        ...
        # values = [Row, ID, JobTitle]
        jid = str(values[1]).strip()
        title = str(values[2])

        self.job_id_var.set(jid)
        self.job_title_var.set(title)
